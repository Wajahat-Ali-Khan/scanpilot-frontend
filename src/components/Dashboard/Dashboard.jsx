...
function Dashboard({ user }) {
  ...
  const metrics = [
    ...
    { label: 'Success Rate', value: stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) + '%' : '0%', color: 'from-purple-600 to-purple-400' }
  ];
            ...
            {recentResults.map((result, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${result.status === 'completed' ? 'bg-green-600' : 'bg-yellow-600'}`} />
                  <span className="text-gray-700">Analysis completed</span>
                </div>
